...
/// **认知 / 意图 / 状态**完整示范：
///
/// - `bundle()` 提供**认知** `BounceCount`（`Memory`）和**意图** `Elasticity`（`Intent`）的初值
/// - `tick()` 从快照读取**状态** `DidBounce`，更新自身认知 `BounceCount`，
