...
- 宿主侧按稳定键/分组/单例查询实体的需求，本质上是“读模型/运行时投影”能力，不宜直接并入 `World` 核心查询面
- `World` 核心应保持“仿真容器 + 生命周期/时间/事件骨架”定位，避免演变为业务查询容器
- 若多个项目反复出现同构需求，可考虑放入可选扩展层：基于实体生命周期和只读读取能力维护宿主索引；索引内容（业务键、分组、单例语义）仍由应用定义
- 短期优先提供宿主指导文档，而非在核心层新增通用索引 API
