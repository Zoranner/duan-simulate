...
**关闭确认（framework-consumer）**：已验证 `guides/custom-domain.md` 的"on_attach / on_detach 生命周期钩子"小节已就绪，明确了调用时机、参数语义、合法与不适合的操作，并附带追踪历史缓存完整示例，之前"不敢用钩子、退而懒初始化"的信心不足问题已彻底消除。
