...
**关闭确认（framework-consumer）**：已验证 `guides/custom-domain.md` 的"域的写入边界"小节已就绪，完整说明了不允许 `compute()` 直接 spawn 的两条核心理由（可追溯性与并行化安全），并附有"事件 + `step_with` 回调"的完整代码示例；`SpawnCommand` 路线图议题属于长期架构讨论，不影响当前使用，短期文档诉求已充分满足。
