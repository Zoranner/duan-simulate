...
- **三元语义（认知 / 意图 / 状态）**：所有数据按语义归入 `Memory`、`Intent`、`State` 三类；中文术语固定为**认知**、**意图**、**状态**，与上述 Rust trait 一一对应，写入权限由类型本身决定，编译期强制
...
**术语约定：** 中文统一使用**认知**、**意图**、**状态**三词；在代码中分别对应 trait `Memory`、`Intent`、`State`（宏 `memory!` / `intent!` / `state!` 亦按此命名）。

所有附加在实体上的数据按语义归入三类，写入权限编译期强制：

```
认知（Memory）：实体的内部认知，对外绝对封闭。
               外部既不能读也不能写，是"我是谁"的载体——
               经验积累、内部推理的中间结果等。

意图（Intent）：实体向外界表达的意志与诉求。
...
|---|---|---|---|---|
| **认知**（`Memory`） | 读/写 | 不可见 | 编译错误 | 编译错误 |
| **意图**（`Intent`） | 读/写 | 只读（快照） | 编译错误 | ✓（在 Reads 中） |
| **状态**（`State`） | 只读（快照） | 只读（快照） | ✓（在 Writes 中） | ✓（在 Reads 中） |
...
```
Entity::tick()  →  意图（Intent）  →  Domain::compute()  →  状态（State）
（单向不可逆，编译期保证）
```

- Entity 可写**认知**（`Memory`）与**意图**（`Intent`），**不能写状态**（`State`，编译期阻止）
- Domain 只能写 `type Writes` 声明的**状态**（`State`）类型，读**意图**/**状态**（`Intent`/`State`，编译期保证）
- **认知**（`Memory`）对外完全不可见，Domain 无法访问任何实体的认知数据
...
### Component：认知 / 意图 / 状态三元语义，按类型密集存储
...
// 用户声明语义的三个公开 trait
pub trait Memory: Component {}   // 认知：实体内部认知，对外封闭
pub trait Intent: Component {}   // 意图：实体对外表达的意志
pub trait State:  Component {}   // 状态：世界法则下的客观条件
...
pub trait Entity: 'static {
    /// 每帧调用。静态方法，无 &self。
    /// 读自身认知/意图做决策，读快照感知状态；写认知（Memory）更新内部模型，写意图（Intent）表达意志。
...
impl Entity for Soldier {
    fn tick(ctx: &mut EntityContext) {
        let mem = ctx.get::<SoldierMemory>().unwrap();  // 认知（Memory），读当前帧值
        let pos = ctx.get::<Position>().unwrap();        // 状态（State），读上帧快照

        // 通过快照感知周围世界（只能看到意图与状态，看不到其他实体的认知）
        let threats = ctx.world().query_nearby::<Position>(pos, 500.0);
        let new_mem = decide_threats(mem, &threats);

        // 写认知（Memory）与意图（Intent）
        // 写状态（Position、Health 等 State）→ 编译错误
        ctx.set(new_mem);
...
|---|---|
| 框架看不到认知数据 → 快照断裂 | 所有数据归入认知/意图/状态 → 快照可管理 |
| `Box<dyn Entity>` 存储 → 缓存不友好 | 函数指针存储 → 8 字节/实体 |
| tick 读写同一存储 → 确定性未定义 | 冻结快照 + pending buffer → 天然确定 |
| tick 可并行性差 | embarrassingly parallel |

**认知的封闭性由类型系统保证**：`SoldierMemory: Memory` 决定了它对 Domain 和 `WorldSnapshot` 完全不可见，不依赖任何架构约定或 debug 检查。
...
**意图**在类型上体现为 `Intent` trait。意图 **按域消费契约设计**，不按实体类型拆分。所有想移动的实体输出同一种意图类型：
...
| 写入边界 | 运行时检查 | 编译期类型保证（认知/意图/状态三元语义 + InWrites/InReads） |
