...
This shape is the acceptance sample for example packages. `ITEM_ID`, `schema()`, and `catalog_item()` are generated by the macros for compatibility and inspection. The stable package metadata contract is `duan::catalog::CatalogItem`; package authors should not write any of these by hand for normal items.
...
- Public contracts: keep hardening `CatalogItem` and related metadata traits so generated descriptors do not become the user-facing contract.
