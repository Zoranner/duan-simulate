...
This document describes the intended authoring surface for package authors. The current framework still needs macro and collection support before every example below can be compiled as-is.
...
This target shape is the acceptance sample for framework work. The current compiling example packages still use explicit `ITEM_ID`, `schema()`, and `Package::builder` calls until these macros and collection APIs exist.
...
- Macro crate: add a proc-macro crate for `derive(Component)`, `derive(Event)`, and attributes `component`, `field`, `event`, `entity`, `domain`, `reaction`, and `observer`.
...
- Item collection: add distributed package item collection for the current Cargo package so users do not maintain a package item list manually.
