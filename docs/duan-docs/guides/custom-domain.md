...
在计算阶段，域可以通过 `ctx.get_own_entity_mut(entity_id)` 修改**自身管辖实体**的组件状态（这是域权威性的直接体现），同时可以向事件通道追加事件。但**不能在 `compute()` 中直接发起生命周期操作（创建/销毁实体）**，生命周期操作通过事件系统在事件处理阶段完成。

框架在类型层面强制执行写入边界：`ctx.get_own_entity_mut(id)` 会在写入前校验目标实体的域归属，非本域管辖的实体返回 `None`。全量只读遍历仍通过 `ctx.entities()` 提供，不受此限制（只读访问不违反权威边界）。
...
fn compute(&mut self, ctx: &mut DomainContext) {
    ...
    for entity_id in entity_ids {
        ...
        // 再可变借用写回（框架校验：entity_id 必须属于本域管辖）
        if let Some(entity) = ctx.get_own_entity_mut(entity_id) {
...
先 `collect` entity_ids 同理——这样做是为了释放对 `ctx.own_entities` 的借用，以便后续可变访问。这是处理 Rust 所有权的标准写法，不是框架的额外限制。
...
fn compute(&mut self, ctx: &mut DomainContext) {
    // 方式一：按实现类型查找（推荐）
    // 返回 Option<&T>，编译期类型安全，无需手动 downcast
    let faction = ctx
    ...
    for observer_id in observer_ids {
        // ... 遍历目标，调用阵营域服务
        // 注意：服务方法需要由调用方传入实体存储，见下方"服务方法签名约定"
...
若同一类型注册了多个具名实例（如 `"red_command"` 和 `"blue_command"` 均为 `CommandRules`），
`get_domain::<T>()` 只能命中最后注册者，此时需要按名称精确定位：

```rust
// 方式二：按名称精确定位（适用于同一类型有多个具名实例）
// 同样返回 Option<&T>，无需手动 downcast
let blue_cmd = ctx
    .get_domain_by_name::<CommandRules>("blue_command")
    .expect("蓝方指挥域未注册");
```

若需要访问域的元数据（如 `domain.entities` 实体列表），可使用裸引用接口：

```rust
// 获取原始域引用（访问域元数据；若需调用服务方法，优先使用上述两种方式）
if let Some(domain) = ctx.get_domain_by_name_raw("faction") {
    // domain: &Domain，可访问 domain.name、domain.entities 等元数据
    // 若需访问规则方法，可通过手动 downcast：
...
两种命名查找方式的区别：
- `get_domain_by_name::<T>(name)` —— 按名称+类型查找，直接返回 `Option<&T>`，是常规用法
- `get_domain_by_name_raw(name)` —— 按名称查找，返回 `Option<&Domain>`，仅用于需要访问域元数据的动态场景

**遍历潜在目标**：

探测是跨实体的交叉计算：观察者在本域管辖范围内，目标则可能是任意活跃实体（包括未归属探测域的实体）。

`ctx.entities()` 提供对全部实体的只读访问能力，全量只读遍历不违反权威边界（写入权限仅限自身管辖实体）：
